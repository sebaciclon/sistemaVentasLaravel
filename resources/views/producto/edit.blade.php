            ...
            <form action="{{ route('productos.update', ['producto' => $producto]) }}" method="POST" enctype="multipart/form-data"> 
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select title="Seleccione una categoría" data-size="5" data-live-search="true" name="categoria_id" id="categoria_id" class="form-control selectpicker show-tick">
                            @foreach($categorias as $tipo)
                                <option value="{{$tipo->id}}" @if ($tipo->id == $producto->categoria->id) selected @endif>{{$tipo->nombre}}</option>
                            @endforeach
                        </select>
                        @error('categoria_id')
                    ...
                    <div class="col-md-8">
                        ...
                        <textarea rows="2" name="descripcion" id="descripcion" class="form-control">{{old('descripcion', $producto->descripcion)}}</textarea>
                        @error('descripcion')
                            <small class="text-danger">{{ '*'. $message }}</small>
                        @enderror
                    </div>
